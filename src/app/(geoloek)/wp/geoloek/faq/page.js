import styles from "@/app/page.module.css";

export default function Page() {
    return(
        <>
            <h1 className={styles.BigHeading}>FAQ</h1>
            <h2 className={styles.SmallHeading}>Wo muss ich hin?</h2>
            <div className={styles.Textblock}>
            Grundsätzlich kannst du mit allen Fragen und Problemen erst einmal zu uns kommen – 
            frag dich einfach zur Fachschaft durch! Das Gleiche gilt auch fürs <a href="https://www.uni-muenster.de/Frontoffice-geoloek/" target="_blank">Front Office</a>. 
            Bei spezielleren Anliegen (z.B. BaFöG-Nachweise, Anrechnungen von Leistungen..) 
            hilft oft der Gang zur Studienfachberatung. Studienberater*innen sind:
            </div>
            <ul>
                <li>Geographie (B.Sc.): <a href="https://www.uni-muenster.de/Geographie/mitarbeiter/luetke.shtml" target="_blank">Dr. Petra Lütke</a></li>
                <li>Geographie (M.Sc.): <a href="https://www.uni-muenster.de/Geographie/mitarbeiter/krajewski.shtml" target="_blank">Dr. Christian Krajewski</a></li>
                <li>Geographie (2-Fach-BA): <a href="https://www.uni-muenster.de/Geographie/mitarbeiter/linnemann.shtml" target="_blank">Dr. Kirsten Linnemann</a></li>
                <li>Geographie (HRSGe): <a href="https://www.uni-muenster.de/Geographiedidaktik/personen/wiss-mitarbeiter/schwarze_start.shtml" target="_blank">Dr. Sonja Schwarze</a></li>
                <li>Geographie (MaEd Gym/Ges & HRSGe): <a href="https://www.uni-muenster.de/Geographiedidaktik/personen/wiss-mitarbeiter/schwarze_start.shtml" target="_blank">Dr. Sonja Schwarze</a></li>
                <li>Landschaftsökologie (B.Sc. & M.Sc.): <a href="https://www.uni-muenster.de/Bodenoekologie/team/hamer.shtml" target="_blank">Dr. habil. Ute Hamer</a></li>
                <li>Wasserwissenschaften (M.Sc.): <a href="https://www.uni-muenster.de/GeoPalaeontologie/boeck.html" target="_blank">Dr. Elke Hanenkamp</a></li>
                <li>Geoinformatik: <a href="https://www.uni-muenster.de/Geoinformatics/Studies/course_guideance/index.shtml" target="_blank">siehe hier</a></li>
                <li>Allgemeine Studien: <a href="https://www.uni-muenster.de/Geowissenschaften/studium/allgstudien.html" target="_blank">siehe hier</a></li>
            </ul>
            <hr></hr>
            <h2 className={styles.SmallHeading}>Wo finde ich meine Vorlesungen?</h2>
            <div className={styles.Textblock}>
            Alle Vorlesungen findest du im <a href="https://studium.uni-muenster.de/qisserver/" target="_blank">Vorlesungsverzeichnis (HISLSF)</a>. Einfach
            mal selbst durchklicken. Du kannst dich dabei immer bei Fragen an <a href="/wp/geoloek/kontakt" target="_blank">uns</a> und bei Problemen an das <a href="https://www.uni-muenster.de/PAMathNat/" target="_blank">Prüfungsamt</a> wenden.
            </div>
            <hr></hr>
            <h2 className={styles.SmallHeading}>Wie funktionieren Seminarwahlen?</h2>
            <div className={styles.Textblock}>
                <ul>
                    <li>Erstes Semester: Infos und „analoge“ Seminarwahl in der <a href="/wp/geoloek/erstis/erstiwoche" target="_blank">Erstiwoche</a>. Da 
                    klärt sich alles auf! Unbedingt kommen!</li>
                    <li>Grundsätzlich verläuft die Anmeldung zu Veranstaltungen für Bachelor- 
                        und Master-Studiengänge über <a href="https://studium.uni-muenster.de/qisserver/" target="_blank">HISLSF</a>. 
                        Die HISLSF-Anmeldephase ist i.d.R immer drei Wochen vor dem Beginn der vorlesungsfreien Zeit.</li> 
                </ul>
            </div>
            <hr></hr>
            <h2 className={styles.SmallHeading}>Wo komme ich an Altklausuren?</h2>
            <div className={styles.Textblock}>
                <ul>
                    <li>Schau dazu mal beim Reiter <a href="/wp/geoloek/angebote/lernstoff" target="_blank">Lernstoff</a> vorbei.</li>
                    <li>Wir <strong>bitten</strong> euch im Austausch dafür neue Protokolle/Altklausuren abzugeben!!</li>
                    <li>Wichtig: wir benötigen immer aktuelle Protokolle! Für euch und eure Komilliton*innen</li>
                </ul>
            </div>
            <hr></hr>
            <h2 className={styles.SmallHeading}>Wie funktioniert das mit den Nebenfächern?</h2>
            <div className={styles.Textblock}>
                <ul>
                    <li>In der O-Woche stellen wir euch alle Nebenfächer und die Infos zur "Nebenfachwahl" vor</li>
                    <li>Zusammengefasst sind die Nebenfächer auch im <a href="/wp/geoloek/erstis/ersti-heft" target="_blank">Ersti-Heft</a></li>
                    <li>Am besten schaut ihr auf der Homepage des <a href="https://www.uni-muenster.de/Geographie/" target="_blank">IfG</a> oder in eurer <a href="https://www.uni-muenster.de/Geographie/studium/studiengaenge.html" target="_blank">Prüfungsordnung</a> nach</li>
                </ul>
            </div>
            <hr></hr>
            <h2 className={styles.SmallHeading}>Wo finde ich was? (Studierendensekretariat, Dekanat, Prüfungsamt)</h2>
            <div className={styles.Textblock}>
                <ul>
                    <li>Siehe <a href="/wp/geoloek/studieninfos/links" target="_blank">Linkliste</a> auf unserer Hompage</li>
                </ul>
            </div>
            <hr></hr>
            <h2 className={styles.SmallHeading}>In welchem Semester beginnt das Studium regulär?</h2>
            <div className={styles.Textblock}>
                <ul>
                    <li>Geographie (B.Sc./M.Sc.): Nur Wintersemester</li>
                    <li>Landschaftsökologie (B.Sc./M.Sc.): Nur Wintersemester</li>
                    <li>2-Fach-Bachelor (Lehramt): Nur Wintersemester</li>
                    <li>Master of Education: Sowohl im Winter- als auch im Sommersemester</li>
                </ul>
            </div>
            <hr></hr>
            <h2 className={styles.SmallHeading}>Gibt es die Möglichkeit das Studium als Quereinsteiger*in bzw. mitten im Semester zu beginnen?</h2>
            <div className={styles.Textblock}>
            In der Regel ist ein Studienbeginn zum Wintersemester zu empfehlen, 
            jedoch besteht auch die Möglichkeit eines Einstiegs zum Sommersemester. 
            Bei Fragen bitte an den zuständige*n Studienberater*in wenden!
            </div>
            <div className={styles.Textblock}>
            <strong>Anrechnungen von Veranstaltungen:</strong> <a href="https://www.uni-muenster.de/Geographie/studium/FAQ.html" target="_blank">FAQ</a> des IfG beachten und
            jeweilige*n Studienberater*in fragen! (Links siehe oben).
            </div>
            <hr></hr>
            <h2 className={styles.SmallHeading}>Ich studiere außercurricular und möchte an einer Prüfung teilnehmen, wie geht das?</h2>
            <div className={styles.Textblock}>
            Kontaktiere dazu am besten das Geschäftszimmer des jeweiligen Instituts.
            </div>
            <hr></hr>
            <h2 className={styles.SmallHeading}>Was muss ich beim Wechsel des Studienplatzes beachten?</h2>
            <div className={styles.Textblock}>
            Siehe Infoblatt zum 
            Studiengangswechsel <a href="https://www.uni-muenster.de/imperia/md/content/landschaftsoekologie/informationen_f__r_studienplatzwechsler_l__k_2018_04.pdf" target="_blank">Landschaftsökologie</a> und <a href="https://www.uni-muenster.de/imperia/md/content/landschaftsoekologie/studienplatzwechsel_geo_2018_04.pdf" target="_blank">Geographie</a>.
            </div>
            <hr></hr>
            <h2 className={styles.SmallHeading}>Hilfe, wie schalte ich meinen PC an???</h2>
            <div className={styles.Textblock}>
            Im <a href="https://www.uni-muenster.de/ZDMgeo/" target="_blank">ZDM</a> gleich im Erdgeschoss des GEO1 
            bekommt ihr kompetente Hilfe rund um das Thema Computer, Hard-/ Software, Medien usw.
            Das Gleiche gilt auch für das <a href="https://www.uni-muenster.de/IT/" target="_blank">CIT</a>.
            </div>
            <hr></hr>
            <h2 className={styles.SmallHeading}>Wie komme ich in das Uni-WLAN oder an Programme wie ArcGIS?</h2>
            <div className={styles.Textblock}>
            Auf den Seiten des <a href="https://www.uni-muenster.de/IT/services/kommunikation/index.html" target="_blank">CIT</a> gibt 
            es Anleitungen für das Uni-WLAN, VPN, E-Mail, Passwörter usw.
            </div>
            <hr></hr>
            <h2 className={styles.SmallHeading}>Wo bekomme ich diese Entlastungsbescheinigung her?</h2>
            <div className={styles.Textblock}>
            Damit du dich ordentlich von der Uni exmatrikulieren kannst, 
            um dich beispielsweise für den Master an einer anderen Uni einzuschreiben, 
            brauchst du eine Entlastungsbescheinigung.
            </div>
            <div>
            Den <a href="https://www.uni-muenster.de/imperia/md/content/geowissenschaften/entlastungsbescheinigung.pdf" target="_blank">Vordruck gibt es hier</a>. Damit musst du dann 
            zum Labor und in die Bibliothek. Den abschließenden Stempel gibt es bei Frau Kreulich.
            </div>
            <hr></hr>
            <h2 className={styles.SmallHeading}>Ich habe da etwas verloren…</h2>
            <div className={styles.Textblock}>
            …und du findest es noch am ehesten wieder in der FS, beim ZDM, in der Bib 
            oder bei dem Hausmeister bzw. der Hausmeisterin. Kannst auch einen Aushang machen. Und beten.
            </div>
            <hr></hr>
            <h2 className={styles.SmallHeading}>Ich verhungere!</h2>
            <div className={styles.Textblock}>
            Gegenüber vom GEO1 findest du die FH. Dort gibt es im Erdgeschoss 
            ein <a href="https://www.stw-muenster.de/essen-trinken/bistros/denkpause/" target="_blank">Bistro</a>.
            Außerdem kannst du auch zum Coesfelder Kreuz zur <a href="https://www.stw-muenster.de/essen-trinken/mensen/am_ring/" target="_blank">Mensa am Ring</a>. In 
            allen Mensen und Bistros des Studiwerks fungiert dein Studiausweis als Mensakarte.
            Aufladen kannst du die mit Bargeld an Automaten in den Mensen. Das solltest du spätestens in der O-Woche lernen.
            Ansonsten wünschen wir guten Appetit!
            </div>
            <hr></hr>
            <h2 className={styles.SmallHeading}>Ich hab noch sooooo viele Fragen!</h2>
            <div className={styles.Textblock}>
            Für Erstsemester siehe auch die <a href="/wp/geoloek/erstis/erstifragen" target="_blank">Ersti-Fragen</a>. Du 
            kannst aber auch zu unseren <a href="/wp/geoloek/angebote/praesenzzeiten" target="_blank">Präsenzdiensten</a> vorbeischauen
            oder eine <a href="/wp/geoloek/kontakt" target="_blank">E-Mail</a> schreiben.
            </div>
        </>
    )
}